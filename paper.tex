...
\hypersetup{
  hidelinks,
  pdfauthor={Shun Hu},
  pdftitle={Exact Denominator Asymptotics, Finite-Jet Rigidity, and Mobius Disjointness for a Class of Two-Step Triangular Skew Products}
}
...
\title[Exact Denominator Asymptotics and Finite-Jet Rigidity]{Exact Denominator Asymptotics, Finite-Jet Rigidity, and M\"obius Disjointness for a Class of Two-Step Triangular Skew Products}
...
reconstruction and a Pad\'e reduction of the bounded-bandwidth Pr\"ufer inversion problem.
Moreover, every nonminimal bounded-bandwidth lift is forced onto an explicit codimension
filtration of lower-degree boundary strata.
\end{abstract}
...
The third output is finite-jet rigidity. \Cref{thm:central-factorial-basis} shows that the
...
\Cref{prop:pade-reconstruction,prop:boundary-thinness} reduce the normalized Pr\"ufer inversion
problem to a finite-dimensional amplitude-lifting problem and show that any nonminimal lift is
forced onto an explicit lower-degree boundary filtration.
...
basis, bounded-bandwidth reconstruction, and the bounded-bandwidth Pad\'e/Pr\"ufer reduction.
...
&\quad+\Bigl(
\begin{aligned}[t]
&-\frac{\ell^2}{2}u+\frac{\ell^2(\ell^2-4)^2}{48}u^3\\
&\qquad -\frac{\ell^2(\ell^2-4)^2(\ell^2-16)^2}{3840}u^5+O(u^7)
\end{aligned}
\Bigr)J_1(t),
\end{aligned}
\]
...
\section{Finite-Jet Rigidity on Bounded Bandwidth Windows}\label{sec:finite-jet}

The support-two and support-three results of \Cref{sec:geometry} are low-order shadows of a
more systematic finite-jet picture. In this section we isolate the bounded-bandwidth mechanism
behind that picture.
...
\subsection*{Pad\'e reduction and generic-shell injectivity}
...
phase quotient is determined by a finite normalized Pr\"ufer jet. What remains is an
amplitude-lifting problem, and \Cref{prop:boundary-thinness} shows that every nonminimal lift is
forced onto a successively thinner top-coefficient boundary stratum. Thus the residual
bounded-bandwidth fibers are stratified by codimension.
\end{remark}
